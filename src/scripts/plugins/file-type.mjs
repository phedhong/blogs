...
export default function remarkFileType() {
	return function (tree, file) {
		...
		const tags = Object.keys(file.data.astro.frontmatter).includes('tags')
			...
			: [...filetags]
		file.data.astro.frontmatter.tags = tags.filter((tag) => !exculdes.includes(tag))

		const isDraft = file.data.astro.frontmatter.draft ? file.data.astro.frontmatter.draft : true
